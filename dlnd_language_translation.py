...
#     print (len(source_text), type(source_text), source_text)
#     print (target_text)
...
#     print (target_sentences)
    ...
    for sentense in source_sentences:
        source_id_text.append([source_vocab_to_int[word] for word in sentense.split()])
        
    for sentense in target_sentences:
        target_id_text.append([target_vocab_to_int[word] for word in sentense.split()])
...
#         drop = tf.contrib.rnn.DropoutWrapper(lstm, output_keep_prob=keep_prob)
#         return drop
#     cell = tf.contrib.rnn.MultiRNNCell([make_lstm_cell(rnn_size) for _ in range(num_layers)])
...
#                   .format(epoch_i, batch_i, len(source_int_text) // batch_size, train_acc, valid_acc, loss))
            ...
            batches = len(source_int_text) // batch_size
#             print(len(source_int_text), batch_size, batches)
            # Show every <show_every_n_batches> batches
            if (epoch_i * batches + batch_i) % show_every_n_batches == 0:
                print('Epoch {:>3} Batch {:>4}/{} - Train Accuracy: {:>6.3f}, Validation Accuracy: {:>6.3f}, Loss: {:>6.3f}'
                      .format(epoch_i, batch_i, batches, train_acc, valid_acc, loss))
...
def sentence_to_seq(sentence, vocab_to_int):
    ...
    arr = [vocab_to_int[word] if vocab_to_int.__contains__(word) else vocab_to_int['<UNK>'] for word in sentence.split()]
